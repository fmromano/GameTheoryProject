...
Each of the algorithms are defined as follows:  S.M. stands for simple matching. 
 D.A.1 stands for deferred acceptance algorithm with evenly distributed quotas. 
 D.A. 2 stands for the deferred acceptance algorithm who performs a one-to-one 
matching each iteration.  This is the algorithm whose preference matrices change
 each iteration due to jobs being matched and cores being used.  Prop. Alg. stands 
for the proposed algorithm introduced in this paper.
...
In this case, simple matching performs worse compared to everyone else based 
on average available threads used, average cores used, and average resource
 score.  The first deferred acceptance algorithm performed similarly to simple
 matching.  Both the second deferred acceptance algorithm and our proposed 
algorithm performed similarly to each other and better on average than the other 
two for this example.  The first deferred acceptance algorithm performed better 
in terms of resources used and threads used for this example.  The proposed 
algorithm performed its jobs the fastest.
